...
# Modify your 'faculty_allocation' function as follows:
    ...
    if faculty_name:
        try:
            ...
            with myconn:
                ...
                cursor.execute("SELECT room FROM faculty WHERE name = ?", (faculty_name,))
                allocated_room = cursor.fetchone()
                if allocated_room:
                    room_number = allocated_room[0]

                    # Render a template that includes the form, navigation bar, and allocation details
                    return render_template("faculty_allocation.html", faculty_name=faculty_name, room_number=room_number)
                else:
